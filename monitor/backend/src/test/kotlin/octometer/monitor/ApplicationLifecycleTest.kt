 ...
 * starts and stops two times against one temporary folder, so the second
 * start proves the first stop released the database file.
 */
class ApplicationLifecycleTest {

    @Test
    fun `the server opens and closes the store on each start and stop, twice in one folder`() {
        val dataDir = Files.createTempDirectory("octometer-lifecycle-test-").toFile().absolutePath
