...
class RequestGuardTest {
    ...
    fun `a route added only in this test still gets the Host check, with no new code at the route`() =
        testApplication {
            ...
            val allowed = client.get("/api/added-by-test") { header(HttpHeaders.Host, "localhost:7431") }
    ...
    fun `a PUT, a PATCH, and a DELETE without an allowed Origin header each get 403`() = testApplication {
        ...
        val deleteResponse = client.delete("/api/added-by-test") {
            header(HttpHeaders.Host, "localhost:7431")
        }
    ...
    @Test
    fun `the ng serve origin passes in dev mode and gets 403 in prod mode`() = testApplication {
        application {
            module(devConfig())
            routing {
                routePost("/api/added-by-test") { call.respond(HttpStatusCode.NoContent) }
    ...
    @Test
    fun `a rejected request holds the security headers too`() = testApplication {
        application { module(prodConfig()) }
