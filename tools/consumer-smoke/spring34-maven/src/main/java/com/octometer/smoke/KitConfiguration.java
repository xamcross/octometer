 ...
 *
 * <p>{@link #ingestController} is also lazy, so a test that never sends a
 * request keeps {@link #mongoEventLogStore} uninstantiated too (issue
 * #69). This app resolves no user id (the anonymous form of the smoke
 * check); the pilot app of the design document gives a real resolver of
 * its own session.
 */
@Configuration
